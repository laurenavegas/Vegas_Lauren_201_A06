...
[https://github.com/laurenavegas/Vegas_Lauren_201_A06](https://github.com/laurenavegas/Vegas_Lauren_201_A06)
...
heights <- list(78, 81, 78, 86, 85)

for (i in 1:length(heights)) { # the length of the list of heights is the same as the number of players 
  # name for the element 
  height_vec <- c("height" = heights[i])
  # add each element and name to the end of each player's list
  player_info[[i]] <- append(player_info[[i]], height_vec)
}
player_info
```
...
```{r}
# initialize empty list to contain points per game
...
for (i in 1:length(player_info)) { 
  ...
  # add each player's points per game to the specified list
  points_per_game <- append(points_per_game, ppg)
}

# assign each player's points per game their initials
names(points_per_game) <- c(names(player_info))
...
```{r}
rounded_ppg <- c(lapply(points_per_game, function(x) round(x, 2)))
rounded_ppg
```
...
```{r}
apply(mat_rand, 1:2, round_fun <- function(x) {
  if (x >= 1000) {
    return(round(x, -3))
  } else if (x >= 100) {
    return(round(x, -2))
...
```{r}
# initalize vectors to hold the years and median temperatures 
years <- c()
medians <- c()

for (i in 1:nrow(co_temps)) {
  # add each year to the specified vector
  years <- append(years, co_temps$year[i])
  # initalize object to hold the sum of temps in each year
  sum <- 0
  for (j in 3:ncol(co_temps)) {
    sum <- sum + as.numeric(co_temps[i, j])
  }
  # calculate the median temperature 
  med <- sum / 12
  # add each year's median temperature to the medians vector
  medians <- append(medians, med)
}

# store the minimum and maximum median indexes 
max_index <- which.max(medians)
min_index <- which.min(medians)

print(paste0("The maximum median annual temperature of ", round(medians[max_index], 2), " occurred in ", years[max_index], " and the minimum median annual temperature of ", round(medians[min_index], 2), " occurred in ", years[min_index], "."))
```
