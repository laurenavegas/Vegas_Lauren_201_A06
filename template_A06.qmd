...
apply(mat_rand, 1:2, round_fun <- function(x) {
  # round values with 4 digits to the nearest thousands
  if (x >= 1000) {
    return(round(x, -3))
    
  # round values with 3 digits to the nearest hundreds
  } else if (x >= 100) {
    return(round(x, -2))
    
  # round values with 2 digits to the nearest tens
  } else if (x >= 10) {
    return(round(x, -1))
  }
})
...
```{r}
month_cols <- max_temps[,3:14]
average_temp <- colMeans(month_cols[, -1])
average_temp
```
...
medians <- apply(co_temps[, -1], 1, median)
years <- co_temps$year
...
sum_digits <- function(num) {
  ...
  # recursive case
    ...
    # add the last digit and run function again with last digit removed
